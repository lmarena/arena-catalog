...
def report_elo_analysis_results(
    ...
    if rating_system == "bt":
        ...
        # add price control
        if price_control:
            print("In elo_analysis.py: Price control is applied")
            bootstrap_df_price, boostrap_coef_price = compute_bootstrap_price_control(
                battles, num_round=num_bootstrap
            )
            elo_rating_final_price, coef_final_price = compute_price_control(battles)
        else:
            bootstrap_df_price = compute_bootstrap_bt(
                battles, num_round=num_bootstrap, num_cpu=num_cpu
            )
            elo_rating_final_price = compute_bt(battles)
    ...
    # add price control 
    model_order_price = list(elo_rating_final_price.keys())

    model_rating_price_q025 = bootstrap_df_price.quantile(0.025)
    model_rating_price_q975 = bootstrap_df_price.quantile(0.975)
             
    ranking_price = {}
    for i, model_a in enumerate(model_order_price):
        ranking_price[model_a] = 1
        for j, model_b in enumerate(model_order_price):
            if i == j:
                continue
            if model_rating_price_q025[model_b] > model_rating_price_q975[model_a]:
                ranking_price[model_a] += 1
    ...
    leaderboard_table_df = pd.DataFrame(
        {
            ...
            "rating_price": elo_rating_final_price,
            "variance": bootstrap_df.var(),
            "variance": bootstrap_df_price.var(),
            "rating_q975": bootstrap_df.quantile(0.975),
            "rating_q025": bootstrap_df.quantile(0.025),
            "rating_price_q975": bootstrap_df_price.quantile(0.975),
            "rating_price_q025": bootstrap_df_price.quantile(0.025),
            "num_battles": battles["model_a"]
            .value_counts()
            .add(battles["model_b"].value_counts(), fill_value=0),
            "final_ranking": pd.Series(ranking),
            "final_price_ranking": pd.Series(ranking_price),
        }
    )
    ...
    return {
        ...
        "elo_rating_price": elo_rating_final_price,
        ...
        "bootstrap_df_price": bootstrap_df_price,
        ...
        "price_coefficients": {
            "bootstrap": np.vstack(boostrap_coef_price),
            "final": coef_final_price,
        }
        if rating_system == "bt" and price_control
        else {},
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument("--price-control", action="store_true")
    ...
    for cat in args.category:
        ...
        results[cat] = report_elo_analysis_results(
            ...
            price_control=args.price_control,

...
def recompute_final_price_ranking(arena_df):
    # compute ranking based on CI
    ranking = {}
    for i, model_a in enumerate(arena_df.index):
        ranking[model_a] = 1
        for j, model_b in enumerate(arena_df.index):
            if i == j:
                continue
            if (
                arena_df.loc[model_b]["rating_price_q025"]
                > arena_df.loc[model_a]["rating_price_q975"]
            ):
                ranking[model_a] += 1
    return list(ranking.values())
    ...
    for i in range(1, len(lines)):
        ...
        for j in range(len(heads)):
            ...
            for h, v in zip(heads, row):
                if h == "Rank_after":
                    if v != "-" and v != "" and v != np.nan:
                        # v = int(ast.literal_eval(v))
                        v =  ('%.15g' % ast.literal_eval(v))
                    else:
                        v = np.nan   
...
def get_full_table(arena_df, model_table_df, model_to_score):
    ...
    for i in range(len(model_table_df)):
        ...
        row.append(model_table_df.iloc[i]["average_price"])
        # Add delta for price control
        # row.append(99)
...
def get_arena_table(
    ...
    # add price control 
    # arena_df["final_price_ranking"] = recompute_final_price_ranking(arena_df)
    ...
    def process_row(row):
        ...
        if arena_subset_df is not None:
            if not price_control:
                ranking = row.get("ranking_difference", 0)
                result.append(ranking if isinstance(ranking, str) else int(ranking))
            else:
                # note: arena_df = arena_dfs["Overall"]
                # the following line determines what gets output to delta column
                ranking_price = row.get("final_price_ranking")
                result.append(ranking_price if isinstance(ranking, str) else int(ranking))
        else:
            result.append(None)
        
        rank_after = model_info.get("Rank_after", 0)  # Default to 0 if the value is missing
        rank_after_is_none = True if rank_after is None else False
        rank_after = rank_after if rank_after is not None else 0  # Handle None explicitly
        # if price_control and rank_after == 0:
        #     return None
        
        result.extend(
            [
                "N/A" if price_control and rank_after_is_none else int(ranking) - int(rank_after),
                model_info.get("Model", "Unknown"),
                int(round(row["rating"])),
                # int(round(row["rating_price"])),
                f"+{round(row['rating_q975'] - row['rating'])}/-{round(row['rating'] - row['rating_q025'])}",
                round(row["num_battles"]),
                round_decimal(model_info.get("average_price", "-")),
                ...
                # ),
                # model_info.get("Rank_after", np.nan),
                # model_info.get("average_price", "-"),
            ]
        )
        return result

# def compute_price_delta(model_info):
    
    # merge the arena_df with model_table_df
    # arena_model_df = pd.merge(arena_df, model_table_df, left_on='Name', right_on='key', how='left').fillna('-')
    # arena_df["Name"] = arena_df.apply(lambda row: row.name, axis=1)
    # print("names: ", arena_df["Name"])
    # model_table_df is two_dimension dataframe
    # should pull out the model name with rank_after out, make a new df then merge with arena_df
    # print("key: ", model_table_df["key"])
    # print("ranks: ", model_table_df["Rank_after"])
    # arena_model_df = pd.merge(arena_df, model_table_df, left_on="Name", right_on='key', how='left').fillna('-')
    # print("Arena_df columns:", list(arena_df.columns))
    # names = arena_df['Age'].apply(lambda x: x * 2)
    # print("Arena_model_df: ", list(arena_model_df.columns))
...
def round_decimal(value):
    if isinstance(value, (int, float)) and not math.isnan(value):
        ...
        rounded_value = "N/A"  # Default value if NaN or non-numeric
...
def update_leaderboard_df(arena_table_vals):
    ...
    # Only keep the delta one due to the none overall category nature
    elo_dataframe = elo_dataframe.drop(elo_dataframe.columns[1], axis=1)
    
    return elo_dataframe.style.apply(highlight_rank_max, subset=["Delta"])

def update_overall_leaderboard_df(arena_table_vals):
    print("Update overall leaderboard df")
    columns = [
        "Rank* (UB)",
        "Rank (StyleCtrl)",
        "Delta",  # New column for price-controlled rank
        "Model",
        "Arena Score",
        "95% CI",
        "Votes",
    ...
    def compare_func_price_ctrl(row):
        if row["Delta"] is None or row["Delta"] == "N/A": 
            return 0
        if int(row["Delta"]) == row["Rank* (UB)"]:
            return 0
        elif int(row["Delta"]) < row["Rank* (UB)"]:
            return 1
        else:
            return -1
    ...
    # print("Before highlighting red and green")
    # print("Indices red: ", indices_red)
    comparison_price_ctrl = elo_dataframe.apply(
        compare_func_price_ctrl,
        axis=1,
    )
    
    indices_red_sec = [i for i, value in enumerate(comparison_price_ctrl) if value == -1]
    indices_green_sec = [i for i, value in enumerate(comparison_price_ctrl) if value == 1]
    
    # keep price if price control is true
    if style_control or price_control:
        elo_dataframe = elo_dataframe.drop(elo_dataframe.columns[1], axis=1)
        return elo_dataframe.style.apply(
            highlight_red, subset=pd.IndexSlice[indices_red_sec, ["Delta"]]
        ).apply(
            highlight_green, subset=pd.IndexSlice[indices_green_sec, ["Delta"]]
        )
    else:
        elo_dataframe = elo_dataframe.drop(elo_dataframe.columns[2], axis=1)
        return elo_dataframe.style.apply(
...
def build_arena_tab(
    ...
    arena_overall_pc_df = None
    if "Overall w/ Price Control" in arena_dfs:
        arena_overall_pc_df = arena_dfs[
            "Overall w/ Price Control"
        ]  # for incorporating style control on the overall leaderboard
        arena_overall_pc_df = arena_overall_pc_df[
            arena_overall_pc_df["num_battles"] > 300
        ]
    
    def update_leaderboard_and_plots(category, filters):
        # in the future, the style/price control = True/False should be set based on if cat_name is in arena_dfs based on my opinion
        ...
        # Add a toggle for the price control
        global price_control
        if "Price Control" in filters:
            price_control = True
            
            cat_name = f"{category} w/ Price Control"
            
            # Jackie added notes: the following should be the correct way after added the column, eg Overal w/ Price Control, to the table
            # if cat_name in arena_dfs:
            #     category = cat_name
            # else:
            #     gr.Warning("This category does not support price control.")
        ...
        # print("baseline_category: ", baseline_category)
        ...
        # Jackie added notes: In the future, after you allows category changes with price control, you can simplify with this
        # if category != "Overall":
        
        if category != "Overall" or price_control:
            # debugging
            print("Non overall category... and price control value is:", price_control)
            values = update_leaderboard_df(arena_values)
            arena_column_widths = [50, 80, 170, 60, 60, 60, 50, 70, 70]
            arena_datatype = ["number","number","markdown","number","str","number","str","str","number"]
            
            arena_values_final = gr.Dataframe(
                datatype=arena_datatype,
                value=values,
                elem_id="arena_leaderboard_dataframe",
                height=1000,
                column_widths=arena_column_widths,
                wrap=True,
            )
        else:
            print("In overall category")
            values = update_overall_leaderboard_df(arena_values)
        ...
        return arena_values_final, p1, p2, p3, p4, more_stats_md, leaderboard_md
    ...
    with gr.Row():
        ...
        with gr.Column(scale=2): # might be here
            category_checkbox = gr.CheckboxGroup(
                ["Style Control", "Show Deprecated", "Price Control"],
                label="Apply filter",
                info="",
            )
    ...
    elo_display_df = gr.Dataframe(
        datatype=arena_datatype,
        value=arena_vals, # changes to values if want to hide price on initial load
        elem_id="arena_leaderboard_dataframe",
        height=1000,
        column_widths=arena_column_widths,
        wrap=True,

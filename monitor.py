...
def build_data_explorer_tab():
    ...
    with gr.Row():
        html = """            
            ...
            src="https://observablehq.com/embed/ca3310d5574ae3a1?cells=chart" style="margin: auto; display: block;"></iframe>
        """
        chart = """
            <div class="chart"></div>
        """
    ...
    with gr.Row():
        category_selector = gr.Dropdown(
            choices=['Puzzles & Problem Solving',
                    'Queries',
        ...
        output_text = gr.Markdown(elem_id="output_text")

    def update_example(category):
        return f"Selected Category: {category}"
    category_selector.change(update_example, category_selector, output_text)
    ...
    if leaderboard_table_file:
        ...
        with gr.Tabs() as tabs:
            ...
            ***Rank (UB)**: model's ranking (upper-bound), defined by one + the number of models that are statistically better than the target model.
            ...
            with gr.Tab("Data Explorer", id=5, interactive=True):
...
def build_demo(elo_results_file, leaderboard_table_file, arena_hard_leaderboard):
    ...
    with gr.Blocks(
        ...
        with gr.Tabs() as tabs:
            with gr.Tab("Leaderboard", id=0, interactive=True):
